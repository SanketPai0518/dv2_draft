/* =========================================================================
   Internet Adoption & Prosperity — app code (humanised & lightly refactored)
   - Keeps behaviour identical to your current build
   - Fewer repetitions, clearer names, comments only where useful
   ========================================================================= */
...
// Simple "latest by code" reducer used in a few places
const latestByCode = (rows, field) => {
  const map = new Map();
  for (const r of rows) {
    if (!Number.isFinite(r[field])) continue;
    const cur = map.get(r.code);
    if (!cur || r.year > cur.year) map.set(r.code, r);
  }
  return map;
};
...
// OWID long → [{country?, code, year, internet}]
async function loadInternetUsersLong() {
  ...
  // Column resolution that tolerates OWID schema drift
  const norm = s => (s || "").replace(/\u200B/g, "").replace(/[^\p{L}\p{N}]+/gu, " ").trim().toLowerCase();
  const byNorm = new Map(table.columns.map(c => [norm(c), c]));

  const codeCol = byNorm.get("code") || byNorm.get("country code");
  const yearCol = byNorm.get("year");
  const nameCol = byNorm.get("entity") || byNorm.get("country") || byNorm.get("country name");
  const valCol =
    ["share of individuals using the internet", "share-of-individuals-using-the-internet",
     "internet users (share of population)", "individuals using the internet % of population", "value"]
      .map(k => byNorm.get(norm(k))).find(Boolean);
  ...
  // Rescale if 0..1 proportions
  const sample = rows.slice(0, Math.min(400, rows.length));
  const frac0to1 = sample.filter(d => d.internet > 0 && d.internet <= 1).length / sample.length;
  if (frac0to1 > 0.6) rows = rows.map(d => ({ ...d, internet: d.internet * 100 }));

  return rows;
}

// World Bank wide CSV → long rows: {code, year, [fieldName]}
async function loadWBIndicatorCSV(path, fieldName) {
  const txt = await safeFetch(path);
  if (!txt) return null;

  // WB files often have a preamble — find the real header
  ...
  const table = d3.csvParse(lines.slice(headerIdx).map(l => l.replace(/\t|;/g, ",")).join("\n"));
...
/* ========== Continent lookup (geojson first, robust fallback) ========== */
async function buildContinentMap() {
  const fallback = {
    ...
    // TERRITORIES
    HKG:"Asia", MAC:"Asia", TWN:"Asia", SRB:"Europe", XKX:"Europe", GRL:"Americas",
  ...
  try {
    ...
    if (gj?.features?.length) {
      for (const f of gj.features) {
        ...
        if (iso && cont) map.set(iso, String(cont).trim());
      }
    }
  } catch {/* ignore */}
...
const baseLayers = [
  ...
  { data:{ url:GEOJSON, format:{ type:"json", property:"features" } },
    mark:{ type:"geoshape", fill:LAND_BASE },
...
// Utility to extract ISO3 for each feature (works across various property names)
const isoTransforms = [
  {
    calculate:
...
// Internet users map (2021 CSV)
const map2021 = {
  $schema: "https://vega.github.io/schema/vega-lite/v5.json",
  width: 1000, height: 560, background: null, padding: 0,
  ...
  layer: [
    ...baseLayers,
    {
      ...
      transform: [
        ...
        { calculate: "isValid(datum.Value) && datum.Value >= minPct ? 1 : 0", as: "keep" }
      ],
      layer: [
        { transform: [{ filter: "datum.keep == 0" }],
          mark: { type:"geoshape", fill:"#1b2a3b" },
          encoding: { shape:{ field:"geometry", type:"geojson" } } },
        { transform: [{ filter: "datum.keep == 1" }],
          mark:{ type:"geoshape" },
          encoding:{
            ...
            color:{
              ...
              legend:{ values:[0,20,40,60,80,100] } // explicit numeric ticks
            },
            tooltip:[
  ...
  config:{ view:{ stroke:null }, axis: axisTheme }
};

// Gap map spec builder (electricity − internet)
function gapMapSpec(values) {
  // Find symmetric domain around 0 for a fair diverging scale
  const maxAbs = Math.max(...values.map(d => Math.abs(d.gap)), 40);
  const dom = [-maxAbs, 0, maxAbs];

  return {
    $schema:"https://vega.github.io/schema/vega-lite/v5.json",
    width:1000, height:560, background:null, padding:0, projection:{type:"equalEarth"},
    layer:[
      ...baseLayers,
      {
        data:{ url:GEOJSON, format:{ type:"json", property:"features" } },
        ...
        encoding:{
          ...
          color:{
            ...
            scale:{ scheme:"redblue", domain: dom, domainMid:0 },
            legend:{
...
function densitySpec(pcts){
  return {
    $schema:"https://vega.github.io/schema/vega-lite/v5.json",
    width:1000, height:420, background:null,
    data:{ values: pcts.map(pct => ({ pct })) },
      ...
      { mark:{ type:"area", opacity:0.18, color:"#60a5fa" },
        encoding:{
          x:{ field:"pct", type:"quantitative", title:"Internet users (% of population)", scale:{domain:[0,100]},
              axis: axisTheme },
          y:{ field:"density", type:"quantitative", title:"Density", axis: axisTheme }
        } },
...
function prosperitySpec(rows, isLog){
  return {
    $schema:"https://vega.github.io/schema/vega-lite/v5.json",
    width:1000, height:520, background:null,
    data:{ values: rows },
...
async function renderMaps(){
  ...
  const [iu, elec] = await Promise.all([
    loadInternetUsersLong(),
    loadWBIndicatorCSV(WB_ELEC, "elec")
  ]);

  const iuLatest   = latestByCode(iu, "internet");
  const elecLatest = latestByCode(elec, "elec");

  const gapVals = [];
  for (const [code, u] of iuLatest) {
    const e = elecLatest.get(code);
...
async function renderDensity(){
  ...
  try {
    ...
    const draw = async () => {
      ...
      // Prefer exact year; if too sparse, use latest ≤ year
      const byCode = new Map();
      for (const r of iu) {
        if (r.year === yr && Number.isFinite(r.internet)) byCode.set(r.code, r.internet);
      }
      if (byCode.size < 20) {
        const best = latestByCode(iu.filter(r => r.year <= yr), "internet");
        byCode.clear(); for (const [code, rec] of best) byCode.set(code, rec.internet);
      }

      const pcts = [...byCode.values()];
...
async function renderProsperity(){
  ...
  try {
    const [iu, gdp] = await Promise.all([
      loadInternetUsersLong(),
    ...
    const pickLatestLE = (arr, code, field, yr) => {
      let best = null;
      for (const r of arr) if (r.code === code && r.year <= yr && Number.isFinite(r[field])) {
    ...
    const draw = async () => {
      const yr = +yearSel.value || years[0];
      const codes = new Set(iu.map(d => d.code));
      const rows = [];

      for (const code of codes) {
        const iuRec  = pickLatestLE(iu,  code, "internet", yr);
        const gdpRec = pickLatestLE(gdp || [], code, "gdp",      yr);
        if (iuRec && gdpRec && gdpRec.gdp > 0) {
          rows.push({
            country: iuRec.country || gdpRec.country || code,
            code, internet: iuRec.internet, gdp: gdpRec.gdp,
            year: Math.min(iuRec.year, gdpRec.year)
          });
        }
      }
...
async function renderProsperityByContinent(){
  ...
  try {
    ...
    const draw = async () => {
      ...
      for (const r of rows) {
        const g = byCont.get(r.continent) || { continent:r.continent, n:0, internet_sum:0, gdp_sum:0 };
